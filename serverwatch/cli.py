...
def parse_arguments():
    ...
    metric_group.add_argument("--memory", action="store_true", help="Show memory usage only.")
    metric_group.add_argument("--swap", action="store_true", help="Show swap usage only.")
    metric_group.add_argument("--disk", action="store_true", help="Show disk usage only.")
    metric_group.add_argument("--filesystems", action="store_true", help="Show mounted filesystem usage only.")
    metric_group.add_argument("--inodes", action="store_true", help="Show inode usage for --disk-path only.")
    metric_group.add_argument("--disk-io", action="store_true", help="Show aggregate disk I/O counters only.")
    metric_group.add_argument("--temperatures", action="store_true", help="Show hardware temperatures only.")
    metric_group.add_argument("--processes", action="store_true", help="Show process count only.")
    metric_group.add_argument("--system", action="store_true", help="Show host and system information only.")
    metric_group.add_argument("--uptime", action="store_true", help="Show system uptime only.")
    metric_group.add_argument("--load", action="store_true", help="Show load averages only.")
    metric_group.add_argument("--network", action="store_true", help="Show network I/O counters only.")
    metric_group.add_argument("--status", action="store_true", help="Show health status only.")
...
def main():
    ...
    try:
        ...
        if selected_metric is not None:
            ...
            print_selected_metric(name, value, args.json, args.disk_path, args.network_interface)
            return EXIT_HEALTHY
        metrics = collect_metrics(args.warning, args.critical, args.disk_path)
    except (DiskIoUnavailableError, TemperatureUnavailableError, NetworkInterfaceError) as error:
        raise SystemExit(f"serverwatch: error: {error}") from error
    except OSError as error:
        raise SystemExit(f"serverwatch: error: cannot read disk path {args.disk_path!r}: {error}") from error
    if args.json:
